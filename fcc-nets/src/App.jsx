...
// ─── FAMILY LINKING HELPERS (new) ─────────────────────────────
const linkChildToParent = async (parentId, childId) => {
  const parentRef = doc(db, "fccnets", "members", parentId);
  const parentSnap = await getDoc(parentRef);
  if (!parentSnap.exists()) return;
  const parentData = parentSnap.data();
  const currentChildren = parentData.children || [];
  if (!currentChildren.includes(childId)) {
    await setDoc(parentRef, { ...parentData, children: [...currentChildren, childId] }, { merge: true });
  }
};

const unlinkChild = async (parentId, childId) => {
  const parentRef = doc(db, "fccnets", "members", parentId);
  const parentSnap = await getDoc(parentRef);
  if (!parentSnap.exists()) return;
  const parentData = parentSnap.data();
  const children = (parentData.children || []).filter(id => id !== childId);
  await setDoc(parentRef, { ...parentData, children }, { merge: true });
};

const findExistingChild = (members, name, dob) => {
              ...
              {blockCals.filter(b => b.date === netsDate).map(b => {
                ...
                return (
                  <div key={b.id} style={{
                    ...
                    <div style={{
                      background: "rgba(100,116,139,.85)", borderRadius: 4,
        ...
        return (
          <div key={p} style={{
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 10, flex: 1, minWidth: 0 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{
                  fontWeight: 800, color: G.text, fontSize: 15,
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // ─── FAMILY LINKING BACKWARD COMPATIBILITY (Step 2) ───
        let initialMembers = mr.exists()
          ? JSON.parse(mr.data().value).map(normMember)
          : SEED_MEMBERS.map(normMember);

        // Make sure every member has children array + correct memberType (for hybrids)
        initialMembers = initialMembers.map(m => ({
          ...m,
          children: m.children || [],
          memberType: m.memberType || (m.teams?.length > 0 ? "player" : "parent")
        }));
        ...
        setMembers(
          SEED_MEMBERS.map(normMember).map(m => ({
            ...m,
            children: m.children || [],
            memberType: m.memberType || (m.teams?.length > 0 ? "player" : "parent")
          }))
          ...
          <div style={{ flex: 1, padding: "28px 24px" }}>
            {!loginCodeSent ? (<>
              ...
              <button onClick={sendLoginCode} disabled={loginCodeSending}
                style={{
                  width: "100%", background: loginCodeSending ? "#e5e7eb" : G.green,
                    ...
                    {list.map(m => {
                      ...
                      return (
                        <button key={m.id} type="button"
                          ...
                          style={{
                            background: sel ? G.green : away ? "#fffbeb" : G.white,
                          ...
                          {grouped[t].map(m => {
                            ...
                            return (
                              <button key={m.id}
                                ...
                                style={{
                                  background: canAdd ? (away ? "#fffbeb" : G.white) : "#f1f5f9",
  ...
  // ════════════════════════════════════════════════════════════
  if (view === "coachhq" && (can(userRole, "accessMembers") || isCoachMember(currentUser?.name, teams))) {
    ...
    return (
      <Shell sidebar={<SidebarNav view={view} setView={setView} userRole={userRole}
        ...
        <div style={{ padding: "16px" }}>
          ...
          {/* Quick stats */}
          <div style={{
            background: G.cream, borderRadius: 14, padding: "16px",
            border: `1px solid ${G.border}`
          }}>
            <div style={{
              fontWeight: 800, fontSize: 12, color: G.text, marginBottom: 12,
        ...
        <div style={{ padding: "20px 16px", display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ display: "flex", background: G.cream, borderRadius: 12, padding: 4, marginBottom: 20 }}>
            <button
              ...
              style={profileTab === "me"
                ? { flex: 1, padding: 12, background: G.green, color: G.lime, borderRadius: 10, fontWeight: 800 }
                : { flex: 1, padding: 12, background: "transparent", color: G.muted }}>
              My Profile (Player)
            </button>
            <button
              onClick={() => setProfileTab("family")}
              style={profileTab === "family"
                ? { flex: 1, padding: 12, background: G.green, color: G.lime, borderRadius: 10, fontWeight: 800 }
                : { flex: 1, padding: 12, background: "transparent", color: G.muted }}>
              👨‍👧 My Family
            </button>
        ...
        <div style={{ padding: "14px 16px 100px" }}>
          ...
          <div style={{
            ...
            <button onClick={() => setShowMatches(v => !v)}
              ...
              <span style={{
                position: "absolute", top: 2, left: showMatches ? 22 : 2,
                width: 20, height: 20, borderRadius: "50%", background: "#fff",
                transition: "left .2s", display: "block"
              }} />
            </button>
            ...
            return (
              <div key={date} style={{ marginBottom: 18 }}>
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                  ...
                  {/* Sessions column */}
                  <div style={{
                    flex: daySessions.length > 0 ? 3 : 0, minWidth: 0,
                      ...
                      return (
                        <div key={s.id} style={{
                          ...
                          <div onClick={() => setExpandedSessions(prev => ({ ...prev, [sessKey]: !expanded }))}
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{
                                fontWeight: 800, fontSize: 12, color: G.text,
                            ...
                            <div style={{
                              ...
                              {inCount > 0 && (
                                <div style={{ marginBottom: 8 }}>
                                  <div style={{
                                    fontSize: 10, fontWeight: 900, color: G.green,
                              ...
                              {unaccounted > 0 && (
                                <div>
                                  <div style={{
                                    fontSize: 10, fontWeight: 900, color: G.muted,
                                    letterSpacing: 1, textTransform: "uppercase", marginBottom: 4
                                  }}>
                    ...
                    <div style={{ flex: 2, minWidth: 0, display: "flex", flexDirection: "column", gap: 6 }}>
                      {dayMatches.map((f, i) => (
                        <div key={i} style={{
                          ...
                          <div style={{
                            fontWeight: 800, fontSize: 11, color: G.text,
                            lineHeight: 1.3
                          }}>
                            {f.label}
                          </div>
                              ...
                              {awayList.map(m => {
                                ...
                                return (
                                  <div key={m.id} style={{
                                    ...
                                    <span style={{
                                      fontSize: 9, padding: "0 5px", borderRadius: 20,
                                      background: catBg[abs?.category] || "#f3f4f6",
                                      color: catColour[abs?.category] || G.muted
                                    }}>
                                      {abs?.category || "Away"}
      ...
      <Shell>
                  ...
                  return (
                    <div key={t.id} style={{
                      ...
                      <div style={{ padding: "12px 14px", display: "flex", flexDirection: "column", gap: 12 }}>
                        ...
                        <div>
                          <div style={{
                            fontSize: 10, fontWeight: 900, color: G.muted, letterSpacing: 1.2,
                            textTransform: "uppercase", marginBottom: 6
                          }}>
                                ...
                                {t.captain && (
                                  <div style={{
                                    ...
                                    <button onClick={() => {
                                      ...
                                      background: "none", border: "none", cursor: "pointer",
                                      color: "#166534", padding: 0, fontSize: 13, lineHeight: 1
                                    }}>×</button>
                                  </div>
                                )}
                                ...
                                {t.vicecaptain && (
                                  <div style={{
                                    ...
                                    <button onClick={() => {
                                      ...
                                      background: "none", border: "none", cursor: "pointer",
                                      color: G.muted, padding: 0, fontSize: 13, lineHeight: 1
                                    }}>×</button>
                                  </div>
                                )}
                  ...
                  return (
                    <div key={m.id}>
                      ...
                      <div onClick={() => setSelMember(isExpanded ? null : m)}
                        ...
                        <span style={{
                          color: G.muted, fontSize: 14, transition: "transform .15s",
                      ...
                      {isExpanded && (
                        <div style={{
                          background: G.cream, borderBottom: `1px solid ${G.border}`,
                          ...
                          {editingName?.id === m.id + "_name_flat" ? (
                            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                              ...
                              <button type="button" onClick={() => setEditingName(null)}
                                style={{
                                  background: G.white, color: G.muted, border: `1px solid ${G.border}`,
                          ...
                          {editingName?.id === m.id + "_contact_flat" ? (
                            <div style={{
                              background: G.white, borderRadius: 8, padding: "10px 12px",
                              border: `1px solid ${G.border}`
                            }}>
                              <div style={{
                                fontSize: 10, fontWeight: 800, color: G.muted, letterSpacing: 1.2,
                                textTransform: "uppercase", marginBottom: 8
                              }}>Edit Contact</div>
                              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                                ...
                                <div style={{ display: "flex", gap: 6 }}>
                                  <button type="button" onClick={() => {
                                    ...
                                    style={{
                                      flex: 1, background: G.green, color: G.lime, border: "none",
                                      borderRadius: 7, padding: "7px", fontSize: 12, fontWeight: 800,
                                      cursor: "pointer", fontFamily: "inherit"
                                    }}>✓ Save</button>
                                  <button type="button" onClick={() => setEditingName(null)}
                    ...
                    <div key={m.id} style={{
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                        ...
                        {editingName?.id === m.id + "_contact" ? (
                          <div style={{
                            background: G.cream, borderRadius: 8, padding: "10px 12px",
                            border: `1px solid ${G.border}`
                          }}>
                            <div style={{
                              fontSize: 10, fontWeight: 800, color: G.muted, letterSpacing: 1.2,
                              textTransform: "uppercase", marginBottom: 8
                            }}>Edit Contact Details</div>
                            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        {codeModal && (
          <div style={{
            ...
            <div style={{
              background: G.white, borderRadius: 16, padding: 28, maxWidth: 340,
              ...
              {/* Big code display */}
              <div style={{
                background: G.cream, border: `2px dashed ${G.green}`, borderRadius: 12,
                padding: "18px 20px", marginBottom: 16, textAlign: "center"
              }}>
                <div style={{
                  fontSize: 11, fontWeight: 800, color: G.muted, letterSpacing: 1.5,
                  textTransform: "uppercase", marginBottom: 6
                }}>Invite Code</div>
                <div style={{
                  fontSize: 28, fontWeight: 900, color: G.green, letterSpacing: 6,
              ...
              <button onClick={() => setCodeModal(null)}
                style={{
                  width: "100%", background: "transparent", color: G.muted,
